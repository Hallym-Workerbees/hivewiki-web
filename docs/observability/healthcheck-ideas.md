...
즉 "host validation을 약하게 푼다"보다 "health check 요청만 의도적으로 우회시킨다"에 가깝습니다.

## Small Extensions Worth Considering

아래 정도는 확장해도 현재 방향성과 잘 맞습니다.

- readiness 실패 시 어떤 dependency가 실패했는지 더 명확히 노출
- DB/cache readiness latency를 메트릭으로 추가
- health check 관련 동작을 runbook에 짧게 정리

반대로 synthetic check, SLI/SLO, 대규모 alert 설계 같은 내용은 이 문서의 핵심은 아닙니다. 필요하면 별도 운영 문서에서 다루는 편이 맞습니다.
